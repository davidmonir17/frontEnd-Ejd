...
const AddTaskForEmployeeModal = ({ open , onClose,AddTask }:{
    onClose:()=> void,
    open:boolean,
    AddTask: (emp: TaskInterface) => void;
}) => {
    ...
    const handleClose = () => {
      ...
      try {
          ...
          // call backend
           const res = await axios.post(`https://localhost:44339/api/Manager/${user.empiId}/employee/${selectedEmployeeid}/AddtaskToEmployee`,taskvalue, {
           headers: {
             Authorization: `Bearer ${user.Token}`,
           },
         });
...
export default AddTaskForEmployeeModal